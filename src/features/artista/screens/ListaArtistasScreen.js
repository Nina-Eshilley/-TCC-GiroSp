...
export default function ListaArtistasScreen({ onVoltar, onEditar, onNovo }) {
  ...
  async function carregar() {
    ...
    try {
      ...
      console.error(err);
      window.alert('Não foi possível carregar artistas');
    } finally {
      setCarregando(false);
    }
  }
  ...
  async function deletar(id) {
    const confirmar = window.confirm('Deseja deletar este artista?');
    if (!confirmar) return;
  ...
  return (
    <View style={styles.container}>
      ...
      {artistas.length === 0 ? (
        <View style={styles.centro}>
          <Text style={styles.vazio}>Nenhum artista cadastrado</Text>
          <TouchableOpacity onPress={onNovo} style={styles.botaoNovo}>
            <Text style={styles.botaoNovoTexto}>Criar primeiro artista</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <FlatList
          data={artistas}
          keyExtractor={(item) => String(item.id_artista)}
          renderItem={({ item }) => (
            <CardArtista artista={item} onEditar={onEditar} onDeletar={deletar} />
          )}
